"""
CRUD для работы с городами и pending-городами
"""
...
async def get_city(session: AsyncSession, city_name: str) -> Optional[City]:
...
async def create_city(session: AsyncSession, city_name: str, utc_offset: int) -> City:
    """Создать город"""
...
async def create_pending_city(
    session: AsyncSession,
    city_name: str,
    admin_telegram_id: str
) -> Optional[PendingCity]:
...
async def get_pending_cities(session: AsyncSession) -> List[PendingCity]:
...
async def approve_pending_city(
    session: AsyncSession,
    city_name: str,
    utc_offset: int
) -> dict:
    ...
    3. Обновить лиды PENDING_UTC → UNIQUE
    """
