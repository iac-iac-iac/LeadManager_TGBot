...
### Менеджер нажимает один сегмент, а открывается другой

**Симптомы:**
- В меню **«Получить лиды»** на кнопке, например, «Промышленный альпинизм», после загрузки новых лидов или просто со временем приходит другой сегмент или неверный шаг (город / количество).

**Причина (исправлено в 2.4.4):**
- В `callback_data` кнопки передаётся **индекс** строки в списке (лимит 64 байта у Telegram), а обработчик повторно вызывал `get_segments_with_cities()`. Порядок сегментов в этом списке не был зафиксирован (зависел от выдачи агрегирующего запроса и обхода данных), поэтому индекс мог указывать на другую строку после изменений в БД.

**Что сделано в коде:**
- Разбор выбора сегмента/города в `src/bot/handlers/manager_leads.py` опирается на **`segments_list` из FSM** — тот же список, по которому построена клавиатура.
- `get_segments_with_cities` в `src/database/crud/segments.py` возвращает список в **стабильном порядке** (сортировка по имени сегмента).

**Если сегмента нет в списке вообще:**
- Сегменты с малым числом уникальных лидов (порог «хвоста», по умолчанию меньше 10 штук) попадают в строки **«📦 Прочее (Обыч.)»** / **«📦 Прочее (Плюсовики)»** — это отдельная бизнес-логика, а не сбой индекса.

---

### Внутри сегмента выбрали «Прочие», а пришли лиды из других направлений

**Симптомы:**
- Менеджер выбирает, например, **«Промышленный альпинизм»** → город **«📦 Прочие (Обыч.)»** (или «Плюсовики»), а в выдаче или в Bitrix оказываются компании из **других** сегментов.

**Как устроено в интерфейсе (чтобы не путать):**
- **Строка-сегмент** **«📦 Прочее (Обыч.)»** / **«📦 Прочее (Плюсовики)»** — в подписи именно слово **«Прочее»**; это **общий** пул хвостов по всем сегментам (мало лидов в сегменте целиком).
- **Псевдо-город** **«📦 Прочие …»** внутри конкретного сегмента — в подписи **«Прочие»**; это **только** мелкие города этого сегмента (меньше порога по количеству в городе).

**Причина (исправлено в 2.4.5):**
- Для «Проч**ие**» как города в FSM хранится реальный `Lead.segment`, но при вводе количества и при подтверждении выдачи в CRUD для категории «Прочее» иногда **не передавался** `segment`, и запрос расширялся на весь межсегментный пул.

**Что сделано в коде:**
- `segment_filter_for_other_leads_pool()` в `src/database/crud/leads.py` и её использование в `manager_leads.py`, `admin_load/manager_flow.py`, `admin_load/bitrix_flow.py`.
